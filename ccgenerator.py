...
st.set_page_config(page_title="CC Generator PRO v8 - Korea & USA High Trust", page_icon="💳", layout="wide")
...
USA_BANKS = {
    ...
    "Chase": ["414720","414740","403116","406045","411432"],           # High trust
    "Bank of America": ["400906","474480","549191"],
    "Capital One": ["414720","434256"],
    "American Express": ["34","37"],                                   # Amex thường trust cao
...
def generate_card(country, bank_name, custom_bin=None):
    ...
    if custom_bin:
        ...
        prefix = random.choice(banks_dict.get(bank_name, banks_dict[list(banks_dict.keys())[0]]))
    ...
    expiry = (datetime.now() + timedelta(days=random.randint(500, 2800))).strftime('%m/%y')  # expiry xa hơn
...
# ==================== GIAO DIỆN V8 ====================
st.title("💳 CC Generator PRO v8 - Korea & USA High Trust Mode")
st.caption("🚀 Tối ưu AVS + Consistency | Dùng cho test trial (OpenAI, Stripe...) | CHỈ DÙNG TESTING!")
...
with tab1:
    with st.form("generate_form"):
        ...
        with col1:
            ...
            num_cards = st.number_input("Số lượng thẻ", 1, 500, 50)
        with col2:
            if country == "United States":
            ...
            custom_bin = st.text_input("Custom BIN", placeholder="414720 (Chase)")

        preset = st.checkbox("OpenAI Trial High Trust Preset (US + Full AVS)", value=False)
    ...
    if generate_btn:
        with st.spinner("Đang sinh thẻ high trust..."):
            ...
            st.download_button("💾 Tải CSV", csv, f"high_trust_{country.lower()}_{datetime.now().strftime('%Y%m%d')}.csv", "text/csv")

            # Copy format cho OpenAI
            st.subheader("📋 Copy format cho OpenAI Trial")
            for card in cards[:5]:  # hiển thị vài cái
...
st.caption("💡 v8: Full AVS matching + High Trust BIN + Data Consistency | US ưu tiên Chase/Amex | Redeploy để cập nhật")
